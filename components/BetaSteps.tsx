...
type Step = {
  ...
  title: string;
  description: string;
  /** שני השלבים הראשונים הם פעולות — הכרטיס כולו הופך לקישור */
  href?: string;
  linkLabel?: string;
  span: string;
  offset: string;
  /** השלב האחרון מקבל מבטא ניאון כדי לסמן את קו הסיום */
  accent?: boolean;
};

/*
 * הכרטיסים יורדים במדרגות ומצטמצמים ברוחב משלב לשלב (5 → 4 → 3 עמודות).
 * ב-RTL הקריאה היא מימין לשמאל, כך שהעין יורדת באלכסון יחד עם המספור.
 */
const STEPS: Step[] = [
  {
    number: "01",
    ...
    >
      <div className="mx-auto w-full max-w-6xl">
        ...
        >
          ...
          >
            איך מצטרפים{" "}
            <span className="font-display font-normal text-neon">לבטא</span>?
          </motion.h2>
          <motion.p
            className="mt-5 text-lg leading-relaxed text-mist"
            variants={fadeUp}
          >
            שלושה שלבים, כמה דקות, ואתם בפנים.
          </motion.p>
        </motion.div>
